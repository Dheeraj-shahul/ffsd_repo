...
exports.getOwnerDashboard = async (req, res) => {
  try {
    ...
    const tenantIds = properties
      .filter(property => property.tenantId && mongoose.Types.ObjectId.isValid(property.tenantId))
    ...
    tenants.forEach(tenant => {
      const property = properties.find(prop => prop.tenantId && prop.tenantId.toString() === tenant._id.toString());
    ...
    const maintenanceRequests = await MaintenanceRequest.find({ tenantId: { $in: tenantIds } });
    const complaints = await Complaint.find({ tenantId: { $in: tenantIds } });
    const agreements = await Agreement.find({ ownerId: objectId });
    const notifications = await Notification.find({ _id: { $in: owner.notificationIds } });

    // Sample reports data
    const reports = {
      monthlyRevenue: payments.reduce((sum, p) => sum + (p.amount || 0), 0),
      occupancyRate: properties.length ? (properties.filter(p => p.isRented).length / properties.length * 100) : 0,
    ...
    const paymentSummary = {
      monthlyRevenue: payments.reduce((sum, p) => sum + (p.amount || 0), 0),
      upcomingPayments: payments.filter(p => p.status === 'Pending').reduce((sum, p) => sum + (p.amount || 0), 0),
